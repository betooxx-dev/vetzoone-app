...
import '../../../blocs/medical_records/medical_records_bloc.dart';
import '../../../blocs/medical_records/medical_records_event.dart';
import '../../../blocs/medical_records/medical_records_state.dart';
...
class _MedicalRecordPageState extends State<MedicalRecordPage>
  ...
  late MedicalRecordsBloc _medicalRecordsBloc;
  ...
  void initState() {
    ...
    _medicalRecordsBloc = sl<MedicalRecordsBloc>();

    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<PetBloc>().add(GetPetByIdEvent(petId: widget.petId));
      _medicalRecordsBloc.add(LoadAllMedicalData(widget.petId));
    });
  }
  ...
  Widget build(BuildContext context) {
    return MultiBlocProvider(
      providers: [BlocProvider.value(value: _medicalRecordsBloc)],
      child: Scaffold(
        body: Container(
  ...
  Widget _buildMedicalRecordsTab() {
    return BlocBuilder<MedicalRecordsBloc, MedicalRecordsState>(
      builder: (context, state) {
        ...
        } else if (state is MedicalRecordsAndVaccinationsLoaded) {
          final medicalRecords = state.medicalRecords;
          if (medicalRecords.isEmpty) {
            return _buildEmptyState(
              'No hay expedientes médicos',
  ...
  Widget _buildVaccinationsTab() {
    return BlocBuilder<MedicalRecordsBloc, MedicalRecordsState>(
      builder: (context, state) {
        if (state is VaccinationsLoading || state is MedicalRecordsLoading) {
          return const Center(
            child: CircularProgressIndicator(color: AppColors.secondary),
          );
        } else if (state is VaccinationsLoaded) {
          final vaccinations = state.vaccinations;
          if (vaccinations.isEmpty) {
            return _buildEmptyState(
              'No hay vacunas registradas',
              'Las vacunas aparecerán aquí',
          ...
          return ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return _buildExpandableVaccinationCardFromEntity(vaccine);
            },
          );
        } else if (state is MedicalRecordsAndVaccinationsLoaded) {
          final vaccinations = state.vaccinations;
  ...
  // Agregar nuevo método para manejar entidades de Vaccination:
  Widget _buildExpandableVaccinationCardFromEntity(Vaccination vaccine) {
    final administeredDate = vaccine.administeredDate;
    final nextDueDate = vaccine.nextDueDate;
  ...
  Widget _buildExpandableMedicalRecordCard(MedicalRecord record) {
